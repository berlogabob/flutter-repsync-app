...
class BandDataFixer {
  ...
  Future<bool> fixBand(String bandId) async {
    try {
      ...
      final needsFix = !_listsEqual(existingMemberUids, memberUids) ||
      ...
      print('✅ Fixed band "$bandId": ${adminUids.length} admins, ${editorUids.length} editors, ${memberUids.length} members');
      return true;
  ...
  bool _listsEqual(List<dynamic>? a, List<dynamic> b) {
    if (a == null) return false;
    if (a.length != b.length) return false;
    return Set<String>.from(a.cast<String>()).containsAll(Set<String>.from(b.cast<String>()));
  }
}
